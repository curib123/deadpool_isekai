# Volume 2 Architecture — Merrowgate Arc

**Project:** This Isekai Has a Bug. Unfortunately, It's Me.  
**Volume:** 2  
**Destination:** DEST-002 — Merrowgate  
**Arc Name:** Merrowgate Arc  
**Phase:** 5 — Volume/Arc Architecture  
**Status:** GATE 6 PASS — VOLUME ARCHITECTURE LOCKED / CHAPTER ROADMAP AUTHORIZED  
**Scope:** Complete volume architecture only. No chapter roadmap, scene list, final dialogue, or manuscript prose.

# Soul-Drift / Story-Intruder Retcon Override

The authoritative retcon in `planning/SOUL-DRIFT-STORY-INTRUDER-RETCON.md` supersedes conflicting origin, POV, fourth-wall, Red-development, and power-limit language in this architecture.

Locked interpretation:
- Red enters Merrowgate with the same overwhelming capability he had from the beginning;
- no Stage I/II progression exists;
- no “discipline growth” or moral-development track exists for Red;
- he may choose cleaner or more controlled manifestations because the scene, specialist advice, secrecy, or joke benefits from it;
- Fourth-Wall Pause is available throughout;
- Cassian is dangerous through systems, evidence, timing, distributed action, and other people's vulnerability—not raw power;
- Merrowgate's existing 24-chapter pacing, reversal, climax, and resolution stay intact;
- the independent Hero/Main-Villain storyline continues elsewhere unless a later gate deliberately intersects it.

# Power-Model Retcon Override

The authorial retcon in `planning/RED-JACKAL-OVERPOWERED-RETCON.md` supersedes all Stage I / Stage II / power-ceiling language in this file.

Red enters Merrowgate already overwhelmingly powerful.

Volume 2 tension comes from:
- incomplete information;
- reputation manipulation;
- political/commercial leverage;
- Red's self-imposed games;
- his off-book mistake;
- evidence and timing;
- protecting multiple objectives.

Any chapter-level “bounded manifestation” language describes what Red **chooses to display**, not what he is capable of.

# 1. Volume Purpose

Merrowgate Arc expands the series from frontier survival into dense commercial, maritime, and urban pressure.

Greywake asked whether a frontier could justify sacrificing people to survive.

Merrowgate asks a different local question:

**Who owns a free port when every service that keeps it free can be bought?**

Red Jackal's personal version is:

**If nobody can officially define me, who gets to profit from the story people tell about me?**

The arc must stand as a complete Merrowgate story.

It is not secretly about Savael, the Exterior Needle, the Great Design, the final cosmology, or a hidden world-ending conspiracy.

# 2. Opening State

## 2.1 Post-Greywake Red Jackal

Red enters Volume 2 carrying the complete consequences of Greywake:

- he remains UNDEFINED to the World System;
- he still cannot read Veyran normally;
- he knows multiple ordinary appraisal methods fail to classify him;
- he understands that physical data can be measured while identity data remains absent;
- he knows physical pain does not meaningfully define his combat ceiling;
- he knows reckless choices can injure people who cannot restore themselves like him;
- his Play Logic is already unbounded in raw potential;
- his real development is judgment, information, restraint, and deciding what to reveal;
- he leaves Greywake voluntarily with no class, level, title, citizenship, legendary reward, or chosen-hero status;
- no Greywake companion automatically follows him;
- his reputation exists as fragmented rumor rather than authoritative fame.

## 2.2 Travel to Merrowgate

Red travels out of the Hollow March through ordinary Avarran road and river networks, eventually entering the Brasswater trade sphere and reaching Merrowgate by an existing legal travel route.

The chapter roadmap may choose the exact transfer sequence later.

Travel must:
- take real time;
- require ordinary food, lodging, passage, and money;
- preserve his literacy limitation;
- avoid teleporting him directly from Greywake;
- avoid turning minor transition stops into accidental full arcs.

No future destination is selected by this travel.

# 3. Merrowgate Identity

Merrowgate is an independent Brasswater trade city positioned between Avarran and Kharadi influence.

Its volume identity must remain visible through:

- layered docks;
- chain bridges;
- dry docks;
- cargo cranes;
- warehouses;
- market towers;
- guild flags;
- foreign quarters;
- mixed Valic and Serekh customs;
- shipwright and mercenary subcultures;
- migrant labor;
- merchant reputation;
- private security;
- port bureaucracy;
- constant movement of goods and people.

Merrowgate is commercially independent rather than noble-centered.

People care less about pedigree than:
- contracts;
- solvency;
- reliability;
- cargo;
- reputation;
- who guarantees whose risk.

# 4. Why Red Jackal Becomes Involved

Red does not arrive intending to save Merrowgate.

He becomes involved because:

1. he needs money after ordinary travel costs;
2. he wants access to wider routes and passage options;
3. normal traveler and employment verification still fails on him;
4. off-book dock work pays well precisely because he is difficult to place inside standard liability systems;
5. rumors from Greywake make some people curious, skeptical, or willing to hire him;
6. he enjoys discovering that his reputation can sometimes open doors;
7. he underestimates how quickly a commercial city can turn reputation into a commodity;
8. a profitable irregular job connects him to the central scheme before he understands it.

He is not motivated by civic duty.

His involvement begins as convenience, money, curiosity, ego, and resistance to being commercially categorized.

# 5. Exact Local Conflict

Merrowgate's independence depends on many competing institutions balancing one another:

- Harbor Council;
- merchant guilds;
- shipwright houses;
- Open Hand Caravans;
- independent warehouse operators;
- smugglers;
- private security companies;
- foreign commercial agents.

No single institution is supposed to own the port's movement, inspections, security, debt, and emergency closures.

Over the past year, a sequence of seemingly unrelated incidents has made that decentralized model look increasingly unsafe:

- selective warehouse theft;
- cargo disappearances;
- forged inspection results;
- damaged moorings;
- privateer information leaks;
- false risk notices;
- targeted sabotage of smaller independent operators.

The incidents are real.

The pattern behind them is engineered.

A powerful merchant-security house has been creating and amplifying instability, then profiting from the resulting fear.

After each incident, the same network gains:
- distressed cargo;
- debt claims;
- insurance leverage;
- warehouse contracts;
- private guard contracts;
- political arguments for centralized control.

The scheme's political objective is the proposed **Unified Harbor Security Charter**.

If adopted, the charter would place emergency inspections, berth closures, major warehouse security, and crisis enforcement under one private commercial authority.

Merrowgate would remain legally independent.

In practice, one merchant house would gain the ability to decide what moves, what waits, what is considered dangerous, and who absorbs commercial loss.

# 6. Main Antagonist

## ANTAG-MG-001 — Cassian Venn

**Species:** Human  
**Age:** 47  
**Origin:** Merrowgate  
**Affiliation:** Venn Maritime House  
**Position:** Maritime assurance magnate, private-security patron, Harbor Council member  
**Status:** Volume 2 local antagonist

## 6.1 History

Cassian was raised inside a Merrowgate shipping family that lost vessels, warehouse access, and political standing during repeated foreign pressure and privateer crises.

He rebuilt the family position by combining:
- cargo assurance;
- debt purchasing;
- warehouse investment;
- convoy protection;
- private security.

He genuinely believes Merrowgate's independence survives only because larger powers find the port useful but difficult to control.

He sees fragmented harbor authority as a historic weakness waiting to be exploited.

## 6.2 Worldview

**A free port is only free while nobody outside it can close it. If the city will not centralize enough to protect itself, someone competent must do it for them.**

## 6.3 Goal

Secure passage of the Unified Harbor Security Charter and place Merrowgate's critical crisis infrastructure under Venn Maritime House control.

## 6.4 Methods

Cassian's network uses:

- selective sabotage;
- purchased informants;
- manipulated risk certification;
- deniable smugglers;
- false security intelligence;
- debt acquisition after manufactured losses;
- private guards;
- Harbor Council lobbying;
- commercial pressure;
- planted evidence;
- legitimate rescue/security work used to build public trust.

He does not control every criminal in Merrowgate.

Smugglers, rivals, and foreign agents have their own goals and sometimes disrupt his plans.

## 6.5 Resources

- merchant wealth;
- warehouses;
- insurance/assurance contracts;
- private security personnel;
- council access;
- legal advocates;
- shipping schedules;
- information brokers;
- commercial debt holdings;
- influence over some inspectors and dock supervisors.

## 6.6 Sympathy Boundary

Cassian is not lying about every danger.

Merrowgate genuinely faces:
- foreign economic pressure;
- privateers;
- smuggling;
- maritime monsters;
- trade sabotage;
- fragmented crisis authority.

He has funded real rescues and real port defenses.

His failure is that he now manufactures danger to prove only his preferred system can solve it.

## 6.7 Relationship to Red Jackal

Cassian initially sees Red as:

**a commercially useful irregular person with no stable registry identity and a growing frontier reputation.**

He does not know why Red is UNDEFINED.

He recognizes that Red's lack of ordinary identity makes him useful for:
- deniable labor;
- off-book movement;
- blame;
- spectacle;
- controlled rumor.

As Red becomes harder to direct, Cassian shifts from trying to use his reputation to trying to define it publicly.

## 6.8 Win Condition

Cassian wins if:

- enough port instability becomes publicly undeniable;
- Red or another irregular outsider can absorb blame for key sabotage;
- the Harbor Council invokes emergency authority;
- the Unified Harbor Security Charter passes;
- Venn Maritime House gains practical control over inspections, closures, and security.

## 6.9 Consequence If Victorious

Merrowgate stays open and nominally independent, but a private commercial monopoly gains the power to decide who may move through the port and who carries the cost of "security."

## 6.10 Consequence If Defeated

The scheme is exposed, the emergency charter fails or is revoked, Venn authority is broken, and Merrowgate must reform security without surrendering the port to one house.

Cassian is a local antagonist.

He has no protected cosmology knowledge.

# 7. Local Cast

## CHAR-015 — Captain Iria Voss

**Species:** Human  
**Age:** 41  
**Origin:** Merrowgate  
**Affiliation:** Merrowgate Harbor Watch  
**Role:** civic-security commander / institutional foil

Independent goal:
Keep public harbor authority from becoming an extension of any merchant house.

Strengths:
- crisis command;
- port law;
- boarding actions;
- crowd/security coordination;
- political caution.

Flaws:
- distrusts irregular people;
- relies heavily on provable procedure;
- can hesitate when evidence is incomplete.

Red relationship:
She considers him a dangerous administrative problem before she considers him useful.

She will not become his subordinate.

## CHAR-016 — Samir Kesran

**Species:** Human  
**Age:** 36  
**Origin:** Serekh family established in Merrowgate  
**Affiliation:** independent shipwright cooperative  
**Role:** engineering / harbor-structure specialist

Independent goal:
Keep smaller dry docks and shipwright yards commercially independent.

Strengths:
- rigging;
- hull repair;
- load distribution;
- cranes;
- dock engineering;
- practical maritime judgment.

Flaws:
- proud;
- hostile to merchant financiers;
- takes technical criticism personally.

Red relationship:
He respects useful improvisation but hates solutions that ignore load, balance, or the workers standing underneath them.

## CHAR-017 — Nessa Quill

**Species:** Riverkin  
**Age:** 28  
**Origin:** Merrowgate  
**Affiliation:** independent dock broker / courier network  
**Role:** social navigation / undercity-and-dock access / transactional ally

Independent goal:
Earn enough legitimate contracts to secure a permanent broker charter without becoming owned by a merchant house.

Strengths:
- dock contacts;
- informal routes;
- negotiation;
- multilingual trade speech;
- reading people;
- rapid information movement.

Flaws:
- cuts legal corners;
- values plausible deniability;
- can convince herself that a suspicious job is merely "normal port business."

Red relationship:
She likes that he is difficult to classify because difficult people create profitable work.

She is not automatically loyal to him.

## CHAR-018 — Dalen Marr

**Species:** Human  
**Age:** 32  
**Origin:** Avarran interior  
**Affiliation:** Open Hand Caravans  
**Role:** logistics factor / reputation conduit

Independent goal:
Keep Open Hand cargo moving through Merrowgate without accepting monopoly protection contracts.

Strengths:
- manifests;
- schedules;
- caravan/ship transfer logistics;
- commercial negotiation;
- rumor verification.

Flaws:
- risk-averse;
- sometimes prioritizes network stability over individuals;
- dislikes improvisation he cannot price.

Red relationship:
He has heard contradictory Greywake stories before meeting Red and becomes an early example of reputation arriving before the person.

# 8. Faction Architecture

## Harbor Council

Goal:
Keep Merrowgate independent, profitable, and governable.

Internal split:
Some members favor Venn's charter because they fear outside pressure and port disorder.
Others fear private capture more than temporary instability.

## Merchant Guilds

Goal:
Predictable trade and profit.

They are not one bloc.

Large houses may favor consolidation.
Smaller houses fear being priced out.

## Shipwright Houses / Independent Yards

Goal:
Maintain technical and commercial independence.

They become important because physical port safety cannot be solved by contracts alone.

## Open Hand Caravans

Goal:
Keep cross-border trade routes open without surrendering to a single local monopoly.

They provide a natural bridge between Greywake rumors and Merrowgate commerce.

## Smugglers

Goal:
Profit from fragmented enforcement.

They are neither heroic resistance nor a single villain faction.

Some unknowingly serve Cassian.
Some knowingly take his money.
Some oppose him because monopoly enforcement is bad for business.

## Private Security Companies

Goal:
Paid authority.

Some compete with Venn.
Some are absorbed by him.
Some resist because monopoly ends competition.

## Foreign Agents

Goal:
Advance outside commercial/political interests.

They complicate the conflict but are not the secret authors of the main scheme.

# 9. Red Jackal's Meaningful Mistake

Red must make one causally important mistake that reflects his established flaws rather than making him suddenly foolish.

He accepts a high-paying off-book port job because:

- normal verification is inconvenient;
- the money is useful;
- the irregularity amuses him;
- he likes proving that rules cannot easily stop him;
- he believes he can survive if the job becomes dangerous.

The job involves moving sealed commercial material through a route designed to avoid ordinary manifest scrutiny.

Red recognizes that the job is suspicious.

He does not know it is connected to Cassian's broader scheme.

His mistake is choosing **not to ask enough questions because being outside the paperwork feels like an advantage.**

The moved material later enables or protects one important sabotage operation.

The exact object, route, and chapter placement belong to Gate 7.

The consequence must affect people other than Red.

Regeneration cannot erase it.

# 10. Midpoint Shift

The midpoint revelation is local, evidentiary, and commercial:

**The apparently unrelated port incidents are producing a consistent pattern of profit, debt acquisition, and security influence for the same network.**

The cast does not yet possess complete proof that Cassian ordered every incident.

The story changes from:

**"Merrowgate is chaotic and somebody is exploiting it."**

to:

**"The chaos itself is being shaped into an argument for private control."**

Red realizes his irregular work may have been part of that system.

# 11. Major Reversal

Cassian's network exposes enough evidence of Red's off-book involvement to make him a credible public suspect in a major sabotage event.

The reversal must achieve all of the following:

- Red's Greywake reputation becomes a liability as well as an asset;
- Harbor Watch loses political room to treat him casually;
- Nessa's broker ambitions suffer because she helped route the job;
- an independent dock/crew suffers a serious material and human consequence;
- Cassian gains temporary political momentum;
- emergency consolidation becomes more likely;
- Red cannot solve the problem by leaving without accepting that his choice helped create the opening.

This is not a false accusation in which Red did nothing wrong.

He did not plan the sabotage.

He did choose the suspicious work that enabled part of it.

# 12. Non-Human / Environmental Pressure

Merrowgate must remain physically maritime.

The arc includes real environmental danger through:

- Brasswater storms;
- tides;
- unstable decks and cranes;
- ship collisions;
- water exposure;
- maritime monsters outside or near the port approaches where appropriate.

Cassian does not control the sea.

Natural danger creates pressure his scheme exploits but does not fully command.

Monster use must follow existing ecology and may not become random combat filler.

# 13. Action Escalation

Action escalates by type:

1. travel and arrival pressure;
2. dock work / urban physical hazards;
3. pursuit or confrontation in warehouses and crowded port routes;
4. sabotage investigation under commercial pressure;
5. shipyard or cargo-crane crisis;
6. public-security escalation after Red is implicated;
7. coordinated harbor emergency where infrastructure, weather, ships, guards, workers, and evidence all matter.

The climax is not a single duel.

# 14. Play Logic Architecture

## 14.1 Progression Ceiling

**Volume 2 ceiling: Stage I — Improvised Manifestation.**

No power stage unlocks in this volume because Red has no power stages.

Any increase in precision, repeatability, or restraint is a **choice of presentation/application**, not growth in power or personality.

## 14.2 Approved Volume 2 Development

### PL-MG-1 — Familiar Utility Repeat

Red can recreate a small category of previously understood simple tools with slightly less conceptual drift.

This is improvement, not guaranteed precision.

### PL-MG-2 — Motion-Environment Utility

Red attempts Stage I constructs on moving decks, cranes, bridges, or unstable harbor surfaces.

Motion, noise, fear, and divided attention increase failure risk.

### PL-MG-3 — Sequential Coordination

Red can contribute several separate simple manifestations across a larger group plan.

He cannot maintain a complex multi-part reality system.

Fatigue accumulates.

### PL-MG-4 — Climax Support

During the harbor emergency, Play Logic may:

- brace;
- bridge;
- catch;
- block;
- anchor temporarily;
- redirect one small immediate hazard;
- create time for trained workers or guards.

It cannot:

- calm the whole sea;
- permanently repair the harbor;
- erase all ships or monsters;
- overwrite trade law;
- mind-control crowds;
- reveal Stage II;
- replace Samir's engineering;
- replace Iria's command;
- replace Nessa's access;
- replace Dalen's logistics.

# 15. Regeneration Architecture

Volume 2 continues the Greywake truth:

- Red heals abnormally;
- healing hurts;
- energy is finite;
- drowning, restraint, repeated impact, exhaustion, poison, magic, and other counters can still matter;
- regeneration does not restore judgment or precision;
- regeneration does not heal anyone else.

The port environment creates new ways survivability can fail to equal victory.

Water, restraint, falling loads, and enclosed spaces must remain credible threats.

# 16. Reputation / Aura Architecture

Merrowgate is the first destination where Red's reputation can arrive ahead of him through trade networks.

Required reputation outcomes:

- one Greywake story helps him;
- one exaggerated version creates fear or commercial interest;
- one person dismisses the story entirely;
- one aura-farming attempt succeeds;
- one is ignored because port workers are busy;
- one backfires because a dramatic action increases his perceived liability.

The arc must show that reputation is a resource other people can trade.

# 17. Comedy Architecture

Primary comedy sources:

- merchants trying to assign a price to an impossible person;
- contradictory Greywake rumors;
- Red discovering that a free port has enormous amounts of paperwork;
- Nessa treating suspicious jobs as ordinary business;
- Dalen refusing to accept "trust me" as a logistics category;
- Samir criticizing visually impressive but structurally bad ideas;
- Red attempting dramatic behavior in places where dock workers care more about cargo clearance;
- Stage I interpretation errors caused by moving environments.

No joke quota exists.

Comedy must drop naturally during the major reversal and harbor crisis.

# 18. Character Movement / Stable Red Contrast

## 18.1 Red Jackal

Red does not undergo a moral-development arc.

Volume 2 instead tests the same stable personality under a different system:
- he likes the freedom of informal port work;
- he likes being a rumor people trade;
- he treats Cassian's manipulation as an invitation to play;
- his off-manifest choice creates real consequences;
- he stays because Cassian uses Red's name/story without permission, because Nessa and others have become chosen pieces on Red's board, and because leaving would let Cassian own the narrative;
- he cooperates with specialists when their knowledge makes the game possible;
- he remains self-centered, theatrical, fourth-wall-aware, and aura-farming at the end.

This is **situational pressure and revelation**, not personality reform.

## 18.2 Iria Voss

Moves from:

**procedure protects the port**

toward:

**procedure is only legitimate when public authority can see who benefits from it.**

She does not become anti-law.

## 18.3 Samir Kesran

Moves from treating every financier as the same enemy toward accepting that technical independence requires political alliances as well as craft competence.

## 18.4 Nessa Quill

Moves from plausible deniability as survival toward accepting that refusing to know who benefits from a job can still make her responsible for its consequences.

Her arc parallels Red without becoming identical.

## 18.5 Dalen Marr

Moves from trying to price every risk toward accepting that some forms of concentrated control create risks no contract can safely externalize.

# 19. Mystery Architecture

## 19.1 M-002 — UNDEFINED Identity

Approved development:

Merrowgate independently confirms that Red's identity failure is not a Greywake-local defect.

Different commercial and civic verification processes may disagree in presentation but converge on the same absence.

Allowed reader conclusion:

**The problem follows Red across jurisdictions.**

Not allowed yet:

- soul-registry proof;
- external continuity truth;
- reincarnation mechanism;
- Savael;
- Exterior Needle;
- Great Design cause.

## 19.2 M-011 — Quiet Registration

Volume 2 may introduce only a bounded historical surface clue:

Old trade or technical records may use terminology that does not match modern System-standard language.

First interpretation:
obsolete commercial or scholarly vocabulary.

Protected truth remains hidden.

## 19.3 Foreshadowing

**F-010 — Quiet Registration Records Use Older Terminology**

Status during architecture:

**PLANNED — NOT YET PLANTED**

Gate 7 may assign a planned chapter location.

It remains unplanted until actual manuscript prose contains the clue and later QA verifies it.

No other new F-ID is authorized by this architecture.

Previously planted F-001 through F-004 remain canon and may be reinforced naturally without changing status.

## 19.4 Protected Mysteries

Volume 2 must not reveal:

- M-001 acquisition truth;
- M-003 continuity-translocation truth;
- M-004 self-model origin;
- technical M-007 Fate model;
- Great Design truth;
- Savael;
- Exterior Needle;
- Exterior Echo;
- Exception Covenant;
- Eidrathi endgame material.

# 20. Volume Movement Architecture

These are movements, not chapters.

## Movement I — A Name Arrives Before the Man

Functions:
- complete travel into Brasswater/Merrowgate;
- establish port culture;
- show contradictory Greywake rumor;
- independently re-establish UNDEFINED identity friction;
- Red needs money and route access;
- Nessa and Dalen enter through their own work.

Tone:
adventure, urban curiosity, commercial comedy.

Mystery ceiling:
M-002 reinforcement only.

## Movement II — Everything Has a Price

Functions:
- Red takes dock work;
- shipwright and Harbor Watch competencies become visible;
- port factions compete;
- first sabotage consequences appear disconnected;
- Red enjoys selective benefits of reputation;
- Cassian's public competence and legitimacy are established.

Tone:
busy, social, transactional, exploratory.

## Movement III — Off the Manifest

Functions:
- irregular jobs become more profitable;
- Red accepts the suspicious off-book movement that becomes his key mistake;
- Nessa's corner-cutting is implicated;
- different sabotage events begin to share commercial aftereffects;
- Stage I refinement is demonstrated without stage advancement.

Tone:
fun until unease becomes visible.

## Movement IV — The Risk Is Being Sold

Midpoint functions:
- the profit/debt/security pattern becomes provable enough to change the investigation;
- Venn-linked interests repeatedly benefit;
- Cassian becomes a serious suspect without complete legal proof;
- F-010 may be prepared for later chapter assignment inside legitimate records work;
- Red realizes his job may connect to the scheme.

Tone:
investigation, pressure, less comedy.

## Movement V — The Useful Suspect

Major reversal functions:
- Red's off-book role becomes public;
- a serious sabotage consequence harms workers/port infrastructure;
- Nessa loses standing or freedom of movement temporarily;
- Iria is pressured to treat Red as a security threat;
- Cassian gains emergency-charter momentum;
- Red considers leaving and chooses not to because his own decision helped create the opening.

Tone:
serious, accusatory, consequence-driven.

## Movement VI — Nobody Owns the Harbor

Functions:
- local factions cooperate for incompatible reasons;
- Samir defines the physical vulnerability;
- Iria builds lawful crisis response;
- Nessa obtains route/access intelligence;
- Dalen coordinates cargo/evacuation/logistics;
- Red supports rather than replaces them;
- evidence and physical defense plans converge.

Tone:
focused, tense, restrained humor.

## Movement VII — Merrowgate Stays Open

Climax:
Cassian's network attempts to convert a real harbor emergency into final political proof that only centralized private authority can keep the port functioning.

The climax combines:
- infrastructure danger;
- weather/tidal pressure;
- ships/cargo/workers;
- private security interference;
- public authority;
- documentary evidence;
- Stage I Play Logic support.

Cassian's defeat requires both:
1. keeping the harbor crisis from becoming catastrophic; and
2. proving the instability was deliberately shaped for commercial consolidation.

Red cannot accomplish both alone.

# 21. Local Resolution

By the end of the Merrowgate Arc:

- the Unified Harbor Security Charter is defeated, revoked, or stripped of monopoly powers;
- Cassian loses practical control and enters local legal/commercial consequence;
- Venn Maritime House remains economically real but cannot retain the engineered emergency authority;
- some collaborators are exposed while unrelated smugglers/criminals remain unresolved local problems;
- Harbor Council separates or cross-checks risk certification, security contracting, and emergency closure power;
- smaller shipwright/warehouse operators gain stronger oversight access;
- the port suffers real repair and trade costs;
- Merrowgate remains messy, competitive, and independent rather than becoming purified.

# 22. Red Jackal Identity Consequence

Merrowgate does not solve Red's System identity.

A narrow local civic workaround may emerge:

A person whose System identity cannot validate may still enter a witnessed local contract under a declared name if living witnesses accept legal liability for the record.

This does not:
- create System registration;
- grant a class;
- prove his origin;
- bind other nations;
- make him a citizen automatically.

It gives Merrowgate a practical way to acknowledge agency without pretending the System has an answer.

Red may accept the practical benefit while refusing any claim that the city has defined what he is.

# 23. Departure Condition

Red leaves Merrowgate only after:

- the local consolidation crisis is resolved;
- his own sabotage involvement is publicly distinguished from Cassian's authorship without erasing his mistake;
- Nessa's status is no longer being used as leverage against him;
- Merrowgate has workable local reforms;
- Red has recovered enough to travel;
- he has money, contacts, or multiple passage options.

His departure remains voluntary.

Volume 3 destination remains unselected.

The final departure may point toward possibility, not a named next arc.

# 24. Gate 6 Scope Lock

Locked by this architecture:

- Volume 2 destination: DEST-002 — Merrowgate;
- arc name: Merrowgate Arc;
- local conflict: engineered port instability used to force private security consolidation;
- main antagonist: ANTAG-MG-001 Cassian Venn;
- local cast roles: CHAR-015 through CHAR-018;
- Red's key mistake: knowingly suspicious off-book work without sufficient questions;
- midpoint: profit/debt/security pattern revealed;
- major reversal: Red implicated and consequence becomes public;
- climax type: harbor emergency + evidence + combined competencies;
- power model: already-overpowered / no progression ceiling; chapter displays are voluntary restraint/revelation choices;
- mystery window: M-002 reinforcement + bounded M-011 surface;
- planned foreshadowing: F-010 only;
- departure: local resolution without selecting Volume 3.

Still deferred to Gate 7:

- final chapter count;
- chapter IDs beyond continuing series numbering;
- chapter titles;
- exact chapter placement of movements;
- exact object moved in Red's mistake;
- exact injury assignments;
- exact F-010 chapter;
- exact Play Logic use counts;
- exact scene order;
- final dialogue;
- manuscript prose.

**Official next step:** Gate 7 — Volume 2 / Merrowgate Arc Chapter Roadmap QA.
